...
const parseArrayField = (rawValue) => {
  ...
  if (typeof rawValue === "object") {
    ...
    if (Array.isArray(rawValue.images)) {
      return rawValue.images;
    }
    if (Array.isArray(rawValue.items)) {
      return rawValue.items;
    }
    if (Array.isArray(rawValue.resources)) {
      return rawValue.resources;
    }
    if (rawValue.reference) {
      return [rawValue.reference];
    }

    return Object.values(rawValue)
      .flatMap((value) => parseArrayField(value))
      .filter(Boolean);
  }
...
const normalizeAssets = (rawValue) =>
  parseArrayField(rawValue)
    .map((item) => {
      if (typeof item === "string") {
        return {
          path: item,
          label: item.split("/").pop() || "asset",
        };
      }

      if (item && typeof item === "object") {
        const path =
          item.url || item.path || item.relativePath || item.href || "";
        const label =
          item.name ||
          item.label ||
          item.filename ||
...
const buildAssetUrl = (path) => {
  ...
  // If API base is relative (e.g., '/api'), serve assets from same origin root
...
export default function LevelChallenge() {
  ...
  }, [currentQuestionIndex, assignedQuestions, userAnswers]);

  const loadLevelQuestions = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/challenges/level-questions`,
        {
          params: {
            userId,
            courseId,
  ...
  const loadRestrictions = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/courses/${courseId}/restrictions`
      );
      if (response.data) {
  ...
  const handleSubmitFeedback = async () => {
    try {
      ...
      // Save to backend
      await axios.post(
        `${import.meta.env.VITE_API_URL}/level-completion`,
        completionData
      );
  ...
  const assetsList = useMemo(
    () => normalizeAssets(challenge?.assets),
    [challenge?.assets]
  );
  const hintsList = useMemo(
    () => normalizeHints(challenge?.hints),
    [challenge?.hints]
  );
