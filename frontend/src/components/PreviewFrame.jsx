...
const PreviewFrame = forwardRef(({ code, isRestricted = false, onConsoleLog, isNodeJS = false, autoRun = false, onHistoryChange, initialFile, stdin = "" }, ref) => {
  ...
  // Removed previewKey to prevent remounting
  ...
  useEffect(() => {
    if (initialFile && initialFile !== viewingFile) {
      // Optional: logic to sync if needed, but usually user navigates internally
    }
  }, [initialFile]);

  // Forward logs
  useEffect(() => {
    if (onConsoleLog) onConsoleLog(logs);
  }, [logs, onConsoleLog]);
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (event.data && event.data.type === 'CONSOLE_LOG') {
        ...
        updatePreview(code, event.data.file);
      }
    };
  ...
  const renderPreview = (targetFile) => {
    ...
    let mainHtml = allFiles[targetFile] || allFiles['index.html'] || '';
    ...
    const helperScript = [
      ...
      '      try {',
      '        const res = this.readFileSync(p);',
      '        if (cb) setTimeout(() => cb(null, res), 1);',
      '      } catch (e) {',
      '        if (cb) setTimeout(() => cb(e), 1);',
      '      }',
      '    },',
      '    readdirSync: function(p) { return Object.keys(window.__STUDENT_FILES__); }',
      '  };',
      ...
      '    // Simple CommonJS Shim for local files',
      '    if (m.startsWith("./") || m.startsWith("/")) {',
      '      const cleanPath = m.startsWith("./") ? m.slice(2) : m.slice(1);',
      '      // Try exact match then .js extension',
      '      const fileContent = window.__STUDENT_FILES__[cleanPath] || window.__STUDENT_FILES__[cleanPath + ".js"];',
      '      if (fileContent) {',
      '        const module = { exports: {} };',
      '        const exports = module.exports;',
      '        try {',
      '          // Wrap in function to provide module scope',
      '          const wrapper = new Function("exports", "require", "module", "__filename", "__dirname", fileContent + "\\n//# sourceURL=" + cleanPath);',
      '          wrapper(exports, window.require, module, cleanPath, "/");',
      '        } catch (e) { throw new Error("Error loading module " + m + ": " + e.message); }',
    ...
    // Auto-inject missing core files
    const injectionTags = [];
  ...
  const updatePreview = (codeToRender, fileToView, isHistoryNav = false) => {
    if (isNodeJS) {
      setLogs([{ type: 'system', content: '[Notice] Browser preview disabled for Node.js.', timestamp: new Date().toLocaleTimeString() }]);
      return;
    }
    ...
    // Call render directly instead of setting key
  ...
  useEffect(() => {
    if (isNodeJS) return;
    if (autoRun) {
      updatePreview(code); // Initial or direct update
      return;
    }
